...
class UserController extends Controller
{
    ...
    public function getEdit(Request $request,$id = '') 
    {
        ...
        if ($request->old('user')) {
            $model->fill($request->old('user'));

            //使用者目前所有代理人(前台in_array)
            $user_agents = $request->old('user')['agent'];

            //使用者目前的團隊(前台in_array)
            $user_teams =  $request->old('user')['team'];
        } else {
            //使用者目前所有代理人(前台in_array)
     ...
     */
     public function postUpdate(UserRequest $request) 
     {
        ...
        if ($model->save()) {
            ...
            return Redirect::to('user/edit/'.$model->id)->withErrors(['msg' => '修改成功']);
        } else {
